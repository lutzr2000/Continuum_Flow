...
_ANIMATED_CONSTANT_DEFAULTS = {
    ...
    "MINIMUM_OXYGEN_CONCENTRATION": lambda physics_cfg: float(physics_cfg["fuel"].get("minimum_oxygen_concentration", 0.0)),
...
def build_animation_state(simulation_cfg, dtype=np.float32):
    ...
    for force_cfg in simulation_cfg.get("forces", ()):
        if force_cfg.get("node_type") not in {"BLENDERCFD_FORCE_CONSTANT_NODE", "CONTINUUM_FLOW_FORCE_CONSTANT_NODE"}:
            continue
...
def apply_config(config):
    ...
    physics_params = {
        ...
        "MINIMUM_OXYGEN_CONCENTRATION": physics_constants["MINIMUM_OXYGEN_CONCENTRATION"],
