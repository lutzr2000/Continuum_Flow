"""CPU voxelization of object-local base masks only."""
...
def voxelize_triangles(triangles: Any, delta: float) -> Any:
    ...
    Mesh bounds determine the grid origin and dimensions. ``surface`` marks
    cells whose centers lie within the configured distance of a triangle.
    """
    if triangles.size == 0:
        return None
...
def surface(
    ...
    are projected onto the triangle plane and tested with a half-cell distance
    tolerance.
    """
    size_i = mask.shape[0]
    size_j = mask.shape[1]
    ...
    total_cells = size_i * size_j * size_k

    for flat in prange(total_cells):
        i = flat // (size_j * size_k)
        remainder = flat % (size_j * size_k)
        j = remainder // size_k
        k = remainder % size_k
