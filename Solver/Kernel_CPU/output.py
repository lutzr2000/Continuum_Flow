...
def enqueue_device_output(
    ...
    """
    Copy one sparse CPU frame into shared memory and enqueue it for VDB output.

    A free writer slot is acquired, tile metadata and enabled field pools are
    copied into its shared buffers, and a JSON payload referencing those
    ...
    if used_tile_count > 0:
        for variable_name in output_list:
            fields[variable_name]["array"][:used_tile_count] = sim_fields[
                variable_name
            ][:used_tile_count]
