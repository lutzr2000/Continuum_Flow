...
def build_active_scalar_tile_mask(
    ...
    for n in prange(total_tiles):
        tile_i = n // (tiles_y * tiles_z)
        rem = n - tile_i * tiles_y * tiles_z
        tile_j = rem // tiles_z
        tile_k = rem - tile_j * tiles_z
...
def predict_scalar_fields_semi_lagrangian(
    ...
    for n in prange(total):
        tile_i, tile_j, tile_k, i, j, k, nx, ny, nz = _active_tile_cell_indices(n, u.shape)
