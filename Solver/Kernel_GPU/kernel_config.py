...
ACTIVE_TILE_SMOKE_EPSILON = 1.0e-4
ACTIVE_TILE_FUEL_EPSILON = 1.0e-4
ACTIVE_TILE_FLAME_EPSILON = 1.0e-4
ACTIVE_TILE_TEMPERATURE_EPSILON = 1.0e-3
...
def active_tile_shape(shape, tile_size=ACTIVE_TILE_SIZE):
    ...
    return tuple(
        (int(shape[axis]) + tile_size - 1) // tile_size
        for axis in range(3)
    )


def active_tile_padding_tiles(tile_size=ACTIVE_TILE_SIZE, padding_cells=ACTIVE_TILE_PADDING_CELLS):
