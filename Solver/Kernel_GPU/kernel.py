...
def solver_debug(config,obstacle_base_masks,obstacle_mask,source_base_masks,source_masks,animated_obstacles,animated_sources):
    ...
    while t < t_max:
        ...
        cuda.synchronize()
        _record_debug_timing(timing_stats, "loop_apply_bc", perf_counter() - section_start)
        ...
        cuda.synchronize()
        _record_debug_timing(timing_stats, "loop_active_tiles", perf_counter() - section_start)
        ...
        v, v_work = v_work, v
        w, w_work = w_work, w
        _record_debug_timing(timing_stats, "loop_velocity_swap", perf_counter() - section_start)
        ...
        cuda.synchronize()
        _record_debug_timing(timing_stats, "loop_pressure_solve", perf_counter() - section_start)
        ...
        cuda.synchronize()
        _record_debug_timing(timing_stats, "loop_projection", perf_counter() - section_start)
    ...
    total_runtime = perf_counter() - total_start_time
    average_step = (timing_stats.get("loop_total", 0.0) / step_count) if step_count > 0 else 0.0
    print(f"Solver runtime: {total_runtime:.3f} s")
    print(f"Simulated steps: {step_count}")
