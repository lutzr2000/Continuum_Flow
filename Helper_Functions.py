...
@cuda.jit
def _partial_maxima_velocity(u, v, w, partial_u, partial_v, partial_w, total_size):
    """
    computes per-block partial maxima for the three velocity components on the GPU.

    Each CUDA block scans a strided chunk of the flattened velocity fields,
    accumulates the largest absolute values it encounters and then reduces these
    local maxima in shared memory. One partial maximum per field and block is
    written to the output arrays for a final host-side reduction.

    Args:
        u (device array): x-velocity field
        v (device array): y-velocity field
        w (device array): z-velocity field
        partial_u (device array): per-block output maxima for `u`
        partial_v (device array): per-block output maxima for `v`
        partial_w (device array): per-block output maxima for `w`
    ...
    if tid == 0:
        partial_u[block] = s_u[0]
        partial_v[block] = s_v[0]
        partial_w[block] = s_w[0]
