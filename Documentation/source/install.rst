...
Continuum Flow consists of two parts: the UI part within Blender and the external Python script running the actual simulation. The code is only tested on Windows.
...
7. Wait until the add-on says that the environment is installed. 
8. When you hit bake for the first time after installation, it will take a moment to start since the code needs to be compiled. After that, it starts faster.
