...
def _project_root_directory(anchor_file=None):
    if anchor_file:
    ...
    for candidate in candidate_directories:
        if (candidate / "UI").exists() and (candidate / "Solver").exists():
            return candidate
...
def solver_environment_directory(anchor_file=None):
    return _project_root_directory(anchor_file) / SOLVER_ENV_DIR_NAME


def legacy_solver_environment_directory(anchor_file=None):
    return _project_root_directory(anchor_file) / LEGACY_SOLVER_ENV_DIR_NAME
...
def solver_requirements_path(anchor_file=None):
    return _project_root_directory(anchor_file) / SOLVER_REQUIREMENTS_FILE
...
def install_solver_environment(
    anchor_file=None,
    python_version=DEFAULT_SOLVER_PYTHON_VERSION,
):
    project_root = _project_root_directory(anchor_file)
